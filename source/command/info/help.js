...
module.exports = {
    ...
    run : async (client, message) => {
        ...
        let prefix = await db.get(`prefix_${message.guild.id}_${client.user.id}`) || client.config.prefix
        ...
        .setThumbnail(message.member.user.displayAvatarURL({format : "png", dynamic: true}))
...
if(!message.member.permissions.has("ADMINISTRATOR") && client.config.ownerID !== message.author.id && !admins.includes(message.author.id)) {
        await message.reply({embeds: [generalembed], components: [generalRow] , fetchReply: true});
} else {
    const msg = await message.reply({embeds: [generalembed], components: [row] , fetchReply: true});
